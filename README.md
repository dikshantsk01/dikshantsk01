# Hi there, I'm Dikshant Khobragade 👋

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&pause=1000&color=36BCF7&width=435&lines=Software+Developer;Full-Stack+%2B+AI+Engineer;Competitive+Programmer;IIT+Patna+Dual+Degree)](https://git.io/typing-svg)

I am a Software Developer and Competitive Programmer pursuing an Integrated Dual Degree (B.Tech + M.Tech) in Civil Engineering at **IIT Patna**. I build full-stack, AI-integrated systems and compete in algorithmic contests and national-level hackathons.

## 🚀 About Me

- 🎓 **Education:** Integrated B.Tech + M.Tech at the Indian Institute of Technology (IIT) Patna.
- 💼 **Experience:** Data Analyst Intern at Judix Technologies (Python, ETL pipelines, API automation).
- ⚡ **Current Focus:** Building AI-integrated full-stack systems — deterministic data/analytics pipelines with AI used as a phrasing/reasoning tool, never as the source of truth.
- 🤝 **Collaboration:** Always open to collaborating on system architecture, algorithmic challenges, or weekend hackathons.

## 🛠️ Featured Projects

**[RootLens AI](https://github.com/sridutt15/AIC_2026_FINAL)** — KPI Intelligence-to-Action Engine
Built as team lead for a 3-person team at the Accenture Innovation Challenge 2026. Ingests heterogeneous enterprise data, detects material KPI anomalies, and ranks explanatory drivers deterministically — a single verified LLM call (Gemini) is used only to phrase the final recommendation, never to calculate it.
`FastAPI` `React` `PostgreSQL (Supabase)` `Pandas` `Statsmodels` `Gemini API`

**[NexusAI](https://github.com/dikshantsk01/Nexus-AI)** — Content Orchestration Platform
Self-initiated, solo-built full-stack application with a RESTful backend of 9 authenticated endpoints, including 6 AI-powered workflows for content generation, image processing, and resume analysis.
`React` `Node.js` `Express.js` `PostgreSQL` `Clerk` `Cloudinary` `Gemini`

## 📊 Competitive Programming

500+ algorithmic problems solved across platforms, with regular participation in rated contests.

- **Codeforces:** Pupil | Peak Rating 1287 | 40+ Contests Participated | [Profile](https://codeforces.com/profile/dikshantsk_01)
- **CodeChef:** 3★ | Peak Rating 1610 | 20+ Contests Participated | [Profile](https://www.codechef.com/users/dikshantsk01)
- **LeetCode:** Active daily/weekly contest participant | 150+ Problems Solved | [Profile](https://leetcode.com/u/dikshantsk01/)

## 🏆 Highlighted Experience & Hackathons

- **Amazon MLSS'26:** Selected in the top 3,000 out of 1.34 Lakh+ applicants nationwide for the Machine Learning Summer School.
- **Goldman Sachs India Hackathon (GSIH):** AIR 663 among participants from top engineering colleges across India.
- **AWS SuperHack 2025:** National Semi-Finalist (Top 32 of 500+ teams) — technical team member on Guardian Agent, managing AWS infrastructure and chatbot integration.
- **Integral Cup 2026:** AIR 1,642.
- **IICPC QuantFest Prelims:** AIR 2,834.
- **HackerRank:** Earned the SDE Intern Certification for core DSA, databases, and debugging speed.

## 🛠️ Technical Arsenal

- **Languages:** C/C++, Python, JavaScript, TypeScript, SQL, HTML, CSS
- **Full-Stack:** React.js, Next.js, Node.js, Express.js, FastAPI, REST APIs
- **Databases & Tools:** PostgreSQL, MongoDB, SQLite, Docker, Git/GitHub, Linux, Postman, AWS
- **Technical Coursework:** Data Structures & Algorithms (DSA), Object-Oriented Programming (OOP), Database Management Systems (DBMS), Operating Systems (OS), Computer Networks (CN)

## 📈 GitHub Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=dikshantsk01&theme=react-dark&bg_color=141321&hide_border=true" alt="Dikshant's GitHub Activity Graph" />
</p>

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/dikshantsk01/dikshantsk01/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/dikshantsk01/dikshantsk01/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/dikshantsk01/dikshantsk01/output/github-contribution-grid-snake.svg">
  </picture>
</div>

## 📫 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dikshant-khobragade-1b166727b/)
[![Codeforces](https://img.shields.io/badge/Codeforces-445f9d?style=for-the-badge&logo=Codeforces&logoColor=white)](https://codeforces.com/profile/dikshantsk_01)
[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black)](https://leetcode.com/dikshantsk01)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/dikshantk._21/)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:dikshantkhobragade994@gmail.com)
